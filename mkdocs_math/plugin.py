...
def inject_preamble_markdown(markdown: str, **kwargs) -> str:
    ...
    # Strip LaTeX-only commands that MathJax cannot handle.
    # These are needed for PDF export but crash MathJax parsing,
    # which aborts the entire $$ block and loses all \newcommand defs.
    _LATEX_ONLY = re.compile(
        r'^\\(usepackage|definecolor|surroundwithmdframed|RequirePackage)\b.*$'
        r'|^\\mdfdefinestyle\b[^}]*\{[^}]*\}',  # multi-line block with braced body
        re.MULTILINE | re.DOTALL,
    )

    def _make_preamble_block(content: str) -> str:
        content = _LATEX_ONLY.sub('', content)
        if not content.strip():
            return ''
        return f'<div class="arithmatex" style="display:none">\n$$\n{content}\n$$\n</div>\n\n'

    # Build separate $$ blocks so an error in one doesn't kill the other
    blocks = []

    # Global preamble
    if _preamble_cache:
        blocks.append(_make_preamble_block(_preamble_cache))
    ...
    if page and page.meta:
        ...
        if local:
            blocks.append(_make_preamble_block(str(local)))
